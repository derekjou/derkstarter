...
So you need to create a new MongoDb database on MongoDB Atlas and configure your app
to use it.

1. Create a new database in [MongoDB Atlas](https://cloud.mongodb.com/)
1. Copy the **MongoDb URI** on MongoDB Atlas and replace `<password>`  with the
  password you just created. It should look something like this:

    ```
    mongodb+srv://jordan:mypassword@jordan1-xmznx.mongodb.net/test?retryWrites=true&w=majority
    ```
...
1. Now we are going to store this URI as an environmental variable using a shell script. Create a new file `env.sh` in `week03/day3/horizonstarter` and put the `MONGODB_URI` from the previous step there:
    ...
    ```bash
    export MONGODB_URI="mongodb+srv://jordan:mypassword@jordan1-xmznx.mongodb.net/test?retryWrites=true&w=majority"
    ```
...
to create test data and look at your database in MongoDB Atlas. Note how there's now a
